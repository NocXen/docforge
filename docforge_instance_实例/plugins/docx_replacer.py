...
class DocxReplacer(BasePlugin):
    ...
    def execute(self, **kwargs) -> ExecutionResult:
        ...
        try:
            ...
            # 记录日志
            if plugin_api:
                plugin_api.log_info(f"开始替换Word文档，共 {len(filtered_templates)} 个模板")
            ...
            if field_lengths:
                ...
                for index in range(max_length):
                    for template_path in filtered_templates:
                        try:
                            # 生成输出文件名（添加索引后缀）
                            output_name = self._generate_output_name(template_path, data, index)
                            output_path = Path(output_dir) / output_name
                            
                            # 替换文档（使用指定索引的值）
                            success = self._replace_document(
                                template_path,
                                output_path,
                                data,
                                index,
                ...
                for template_path in filtered_templates:
                    try:
                        # 生成输出文件名
                        output_name = self._generate_output_name(template_path, data)
                        output_path = Path(output_dir) / output_name
    ...
    def _generate_output_name(self, template_path: str, data: Dict[str, List[str]], index: int = None) -> str:
        """
        生成输出文件名
        
        支持在文件名中使用字段，如：{公司名称}_审批单.docx
        ...
        while output_name in self._generated_names:
            output_name = f"{base_name}_{counter}{template_ext}"
            counter += 1
    ...
    def _replace_document(
        ...
        try:
            ...
            if plugin_api:
                plugin_api.log_error(f"替换文档失败: {e}")
            return False
